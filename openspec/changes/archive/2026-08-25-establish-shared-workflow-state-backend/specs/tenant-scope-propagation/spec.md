...
Tenant-scoped governance facts, execution authorizations, workflow context, reusable namespace keys, shared workflow snapshots, leases, and idempotency records SHALL carry the effective tenant scope fingerprint and SHALL reject mismatches. Shared backend keys SHALL never use raw tenant identifiers.
...
- **THEN** their authorization, workflow, lease, or namespace fingerprints differ

#### Scenario: Lease scope mismatch is rejected
- **WHEN** a worker presents a valid workflow lease for a different tenant scope
- **THEN** the shared state backend rejects the mutation and reveals no workflow state
...
Public request and error serialization SHALL expose only bounded opaque scope references or profile metadata, never trusted roles, tenant credentials, raw tenant identifiers, entitlement claims, worker owner identifiers, or fencing tokens.
