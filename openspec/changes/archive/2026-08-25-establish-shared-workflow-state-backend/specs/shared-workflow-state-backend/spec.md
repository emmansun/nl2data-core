...
The backend SHALL use versioned migrations, bounded connection/command timeouts, connection pooling limits, and safe normalized workflow errors. It SHALL reject unsupported newer schemas and SHALL not expose DSNs, credentials, or raw backend exception text.
...
State updates SHALL atomically verify workflow identity, expected revision/version, expected status, tenant scope, and current fencing ownership when supplied. Stale writers SHALL never silently overwrite newer state.
