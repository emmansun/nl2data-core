...
Discovery SHALL use bounded connection/command timeouts and normalize unavailable, unauthorized, malformed, and partial-discovery failures without leaking credentials, DSNs, raw backend exceptions, or raw metadata payloads. The admin API SHALL map these results to stable safe HTTP/service errors and SHALL never claim a snapshot was activated when it was not.
