...
### Requirement: Legacy plans have an explicit compatibility bridge
The planning boundary SHALL provide a deterministic translation from valid `SemanticQueryPlan` values to `SemanticQueryIR` and SHALL preserve existing plan/compiler callers during the supported migration window. New workflow and compiler integrations SHALL normalize to IR before execution.

#### Scenario: Existing plan path remains executable
- **WHEN** an existing static plan resolver returns a valid `SemanticQueryPlan`
- **THEN** the runtime normalizes it to IR and the existing SQL or MongoDB compatibility compiler can execute without changing the governed gates

#### Scenario: Invalid legacy plan cannot bypass IR validation
- **WHEN** a legacy plan contains invalid structure or unsafe physical content
- **THEN** normalization or IR validation rejects it before any adapter call
