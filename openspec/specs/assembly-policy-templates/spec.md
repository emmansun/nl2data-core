# assembly-policy-templates Specification

## Purpose
Define a closed, bounded authoring surface for governance policy templates that expand deterministically into ordinary policy assertions before review, so that documents never carry template state, raw policy payloads, or lifecycle authority into the canonical payload or fingerprint domain.
## Requirements
