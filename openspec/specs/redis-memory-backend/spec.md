# redis-memory-backend Specification

## Purpose
Define an independently installable Redis backend for shared, bounded, tenant-scoped Memory records, implementing the core `MemoryProvider` contract with lazy redis-py loading, atomic compare-and-set, durable TTL, and normalized errors.
