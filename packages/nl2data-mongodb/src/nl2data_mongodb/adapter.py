...
class MongoQueryAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        elif self._config.database is None or (
            self._config.uri is None and self._config.uri_reference is None
        ):
            raise MongoAdapterError(
                "the pymongo profile requires a uri and database",
                details={"profile": "pymongo"},
            )
        else:
            self._executor = PyMongoExecutor(
                self._config.resolve_uri(),
                self._config.database,
                server_selection_timeout_ms=self._config.server_selection_timeout_ms,
            )
