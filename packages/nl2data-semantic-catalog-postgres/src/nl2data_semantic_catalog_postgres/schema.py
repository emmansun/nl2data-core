...
snapshot, proposal-set, Bundle publication, pointer, history, event, and
schema-metadata surface.  A runtime never applies a migration newer than the
schema version it supports, and a database reporting a newer schema than the
runtime supports is rejected without modification.
"""
