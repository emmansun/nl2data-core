...
| `audit evidence entry fingerprint mismatch` (recording) / tampered audit-evidence envelope on read | A persisted or submitted audit-evidence entry no longer matches its fingerprint, or an entry is filed under a foreign tenant/source scope | Fail-closed by design; inspect the [audit-evidence trail](../reference/assembly-audit-evidence.md) from trusted lifecycle facts and re-record identical entries (recording is idempotent) — never edit or bypass validation |
| `audit_evidence_invalid` on activation/rollback | The pointer-change audit-evidence entry could not be built (missing publication entry or unusable scope identity) | The pointer change is rejected before any state moves; resolve the publication evidence for the target version first, then retry the activation or rollback |
| `publication_audit_evidence_mismatch` on reload/reuse/activation/rollback | Durable audit evidence no longer agrees with the immutable publication identities (bundle fingerprint, draft revision, manifest, Verification Suite evidence, policy, scopes) | Treat as an atomic rejection; re-verify the publication artifacts from trusted metadata — never reconstruct missing evidence from mutable drafts |
| Publication reads classify as `legacy` or `incomplete` instead of `complete` | Pre-audit-evidence publication (or partial records) lacks the durable audit-evidence binding | Expected for historical records: `legacy` is readable as history but is not production-valid release evidence; re-publish under the current flow to obtain `complete` evidence. Missing evidence is never fabricated |
