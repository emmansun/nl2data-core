...
class TestErrorNormalization:
    ...
    def test_url_constructor_rejects_missing_driver(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            "nl2data_core.memory.redis_provider.driver_available", lambda: False
        )
        with pytest.raises(MemoryInvocationError) as exc:
    ...
    def test_lazy_client_build_failure_is_normalized(
        ...
        monkeypatch.setattr(
            "nl2data_core.memory.redis_provider.driver_available", lambda: True
        )
    ...
    def test_lazy_exports_never_import_redis(self) -> None:
        from nl2data_core import memory
        ...
        assert memory.RedisMemoryConfig is RedisMemoryConfig
        assert memory.RedisMemoryProvider is RedisMemoryProvider
