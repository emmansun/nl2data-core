...
class TestFieldScope:
    def test_columns_outside_selection_scope_are_removed(self) -> None:
        ...
        protected = protect_result(result, plan=make_plan(), limits=EffectiveLimits())
    ...
    def test_all_columns_out_of_scope_fails_safely(self) -> None:
        ...
        with pytest.raises(ResultProtectionError) as excinfo:
            protect_result(result, plan=make_plan(), limits=EffectiveLimits())
        assert "field scope" in excinfo.value.message

    def test_unaliased_selection_uses_physical_name(self) -> None:
        plan = make_plan(
            selections=(
                SemanticSelection(selection_id="s1", field_id="order_id"),
                SemanticSelection(selection_id="s2", field_id="amount"),
            ),
        )
        result = make_result(columns=("order_id", "amount"), rows=((1, 10.0),))
        protected = protect_result(result, plan=plan, limits=EffectiveLimits())
        assert protected.column_names == ("order_id", "amount")

    def test_unaliased_aggregate_uses_generated_alias(self) -> None:
        plan = make_plan(
            selections=(
                SemanticSelection(selection_id="s1", field_id="amount", aggregation="sum"),
            ),
        )
...
class TestBounds:
    def test_row_bound_is_enforced(self) -> None:
        ...
        with pytest.raises(ResultProtectionError) as excinfo:
            protect_result(result, plan=make_plan(), limits=EffectiveLimits(max_rows=1))
    ...
    def test_column_bound_is_enforced(self) -> None:
        ...
        with pytest.raises(ResultProtectionError) as excinfo:
            protect_result(result, plan=make_plan(), limits=EffectiveLimits(max_columns=2))
...
class TestPublicContract:
    def test_protected_result_is_immutable_public_model(self) -> None:
        protected = protect_result(make_result(), plan=make_plan(), limits=EffectiveLimits())
